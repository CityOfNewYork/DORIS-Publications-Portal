...
mysql -u root -p$DB_PASS -e "UPDATE publications.document SET url = CONCAT('/static/data/gppPdfs/', filename) WHERE url = '';"
mysql -u root -p$DB_PASS -e "DELETE FROM publications.document WHERE title LIKE '%Sample%' AND agency = 'Aging';"
mysql -u root -p$DB_PASS -e "DROP doc_text FROM publications.document;"

# Update /usr/share/elasticsearch/bin/elasticsearch.in.sh
sed -i "4i\ \n# Set ES_HEAP_SIZE\nexport ES_HEAP_SIZE=3gb" /usr/share/elasticsearch/bin/elasticsearch.in.sh
