...
def results(request):
    ...
    context_dict = {'results': request.session['results_list'],
                    ...
                    'selected_agencies': ', '.join(request.session.get('agencies')),
                    'selected_categories': ', '.join(request.session.get('categories')),
                    'selected_types': ', '.join(request.session.get('types')),
                    'pag_res': pag_res,
